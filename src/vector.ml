...
module Make (T : TYPE) : S with type num_type := T.num_type = struct
  ...
  let set v ~index:i ~v:newv = if i < 0 || i >= Array.length v.data then () else unsafe_set v i newv
  ...
  let for_all ~f:pred v =
    let successes = fold_left ~f:(fun ret e -> if pred e then succ ret else ret) ~init:0 v in
  ...
  let copy ?y v =
    ...
    | Some y ->
        iteri ~f:(fun index m -> set y ~index ~v:m) v;
        v

  let equals v1 v2 =
    if Array.length v1.data <> Array.length v2.data then false
    else Array.map2 (fun v1 v2 -> T.compare v1 v2 = 0) v1.data v2.data |> Array.for_all Util.id
end
