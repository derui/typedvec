...
let of_int : int -> (module SIZE) = fun n ->
  let rec fold' n (module F:F) =
    if n <= 0 then (module S(F): SIZE)
    else fold' (pred n) (module struct type size = F.size s let size = (succ F.size) end : F)
  in
  fold' n (module struct type size = z let size = 0 end : F)
