...
let may_tuple tup = function [] -> None | [ x ] -> Some x | l -> Some (tup ?loc:None ?attrs:None l)

let lid s = Location.{ txt = Longident.parse s; loc = !default_loc }
...
let tup_to_list tup =
  let rec tup_to_list' memo = function
    | { pexp_desc = Pexp_construct ({ txt = Lident "[]"; _ }, None); _ } -> List.rev memo
...
let fun_ args e =
  ...
  | pat :: rest ->
      let rec fun_' pats func =
        match pats with
        | []          -> func
        | pat :: rest -> fun_' rest (Exp.fun_ Nolabel None (Pat.var { txt = pat; loc = !default_loc }) func)
      in
...
let names_to_module_path paths =
  ...
  | f :: paths -> List.fold_left (fun ident path -> Ldot (ident, path)) (Lident f) paths
