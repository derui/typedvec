...
let translate af ~vec =
  ...
  Vec.iteri vec ~f:(fun index v -> 
    Mat.set ~row:size ~col:index ~v m
  );
...
let rotate af ~rotate =
  ...
  let m = Mat.map m ~f:(fun row col v ->
    if row >= rotate_size || col >= rotate_size then v
    else Mat.get ~row ~col rotate |> Option.value ~default:v
  ) in
  let open Mat.Open in
  {af with data = af.data *: m}
...
let scale af ~scale =
  let m = Mat.identity (Size.succ af.size) in
  Vec.iteri scale ~f:(fun index v ->
    Mat.set ~row:index ~col:index ~v m
  );
  let open Mat.Open in
  {af with data = af.data *: m}
